...
# we force stderr to avoid synchronisation issues on calculation errors, even
...
reset()
{
	fini  # close old pipes
	sleep 1
	kill -1 $coproc_pid >/dev/null 2>&1  # make sure coproc is dead
	unset coproc_pid
	init
}
...
# print the result. we could run `bc <<<"scale = 10 ; $*"' and be done
